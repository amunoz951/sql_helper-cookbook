...
module SqlHelper
  @debug = false # show queries and returned json
  def self.sql_script_dir; "#{File.dirname(__FILE__)}/sql"; end

  def self.execute_sql(connection_string, sql_query)
    ps_script = <<-EOS
        . "#{Settings.paths['script_dir']}\\libraries\\sql_helper\\sql_helper.ps1"
    ...
    puts IO::popen(['powershell.exe', ps_script]) { |io| io.read }.strip # run sql
  end
  ...
  def self.execute_query(connection_string, sql_query, return_type: :all_tables, values: nil, readonly: false, ignore_missing_values: false)
    ...
    show_queries(sql_query) if @debug
    ps_script = <<-EOS
        . "#{Settings.paths['script_dir']}\\libraries\\sql_helper\\sql_helper.ps1"
    ...
    result = IO.popen(['powershell.exe', "powershell -File \"#{ps_script_file}\""]) { |io| io.read }.strip # run sql
    if result.include?('FullyQualifiedErrorId') || !$?.success?
  ...
  def self.show_queries(sql_query)
    ps_script =
      <<-EOS
        . "#{Settings.paths['script_dir']}\\libraries\\sql_helper\\sql_helper.ps1"
        $sqlCommand = '#{sql_query.gsub('\'','\'\'')}'
  ...
  def self.convert_powershell_tables_to_hash(json_string, return_type = :all_tables) # options: :all_tables, :first_table, :first_row
    IOHelper.logger.debug(json_string) if @debug
  ...
  def self.convert_javascript_dates_to_time_objects(value)
    case value
    when Array
  ...
  def self.connection_string_valid?(connection_string, suppress_failure: true)
    begin
  ...
  def self.get_sql_server_settings(connection_string)
  ...
  def self.get_backup_sql_server_settings(connection_string)
  ...
  def self.insert_values(sql_query, values)
  ...
  def self.backup_fileset_names(backup_files)
    result = ''
  ...
  def self.get_database_size(connection_string, database_name)
  ...
  def self.get_sql_disk_space(connection_string, target_folder)
  ...
  def self.get_sql_backup_headers(connection_string, backup_files)
  ...
  def self.sql_server_backup_files(sql_server_settings, backup_basename)
  ...
  def self.get_backup_files(sql_server_settings, backup_folder, backup_basename)
  ...
  def self.get_backup_size(sql_backup_header)
    sql_backup_header['BackupSize'].to_f / 1024 / 1024
  end

  def self.check_header_date(sql_backup_header, backup_start_time, messages = :none) # messages options: :none, :prebackup | returns: :current, :outdated, :nobackup
  ...
  def self.create_sql_login(connection_string, user, password)
  ...
  def self.export_logins(connection_string, database_name)
  ...
  def self.execute_script_file(connection_string, import_script_filename, values: nil, readonly: false)
  ...
  def self.assign_database_roles(connection_string, database_name, user, roles = ['db_owner'], sid: nil) # roles: array of database roles
  ...
  def self.database_roles_assigned?(connection_string, database_name, user, roles, sid)
  ...
  def self.connection_string_part_regex(part)
    case part
    when :server
  ...
  def self.connection_string_part(connection_string, part, value_only: true) # options: :server, :database, :credentials, :readonly
    case part
    ...
    return $3 if value_only
    result = ($1 || '') + '=' + ($3 || '') + ($4 || '')
    IOHelper.logger.debug("connection_string_part: #{result} \n caller: #{caller.select { |method| method[%r{(embedded/)(lib|bin)}].nil? }.pretty_inspect}") if @debug
    result.empty? ? nil : result
  end

  def self.remove_connection_string_part(connection_string, part) # part options: :server, :database, :credentials, :applicationintent
  ...
  def self.hide_connection_string_password(connection_string)
    credentials = connection_string_part(connection_string, :credentials)
    credentials[:password] = credentials[:password].gsub(/(.)([^(.$)]*)(.$)/) { $1 + ('*' * $2.length) + $3 } if credentials.is_a?(Hash) && !credentials[:password].nil? && credentials[:password].length > 2
  ...
  def self.replace_connection_string_part(connection_string, part, replacement_value)
  ...
  def self.run_sql_as_job(connection_string, sql_script, sql_job_name, sql_job_owner = 'sa', values: nil)
  ...
  def self.ensure_sql_agent_is_running(connection_string)
  ...
  def self.compress_all_tables(connection_string)
  ...
  def self.update_sql_compatibility(connection_string, compatibility_level) # compatibility_level options: :sql_2008, :sql_2012
  ...
  def self.get_unc_backup_files(backup_folder, backup_basename)
  ...
  def self.backup_location_and_basename(connection_string, database_name)
  ...
  def self.backup_basename(backup_path)
  ...
  def self.to_integrated_security(connection_string, server_only: false)
